...
csvpso <- function(
  ...
  vel.c <- matrix(runif(n*control$s,-1,1), ncol=control$s) * max_wgts * control$maxV
  ...
  # init global best
  gb.c <- lb.c[, which.min(lb.fit)]
  
  
  # init global best
  gb.s <- lb.s[, which.min(lb.fit)]
  ...
  while(i<=control$maxit){
    ...
    vel.s <- control$inertia * vel.s + 
      control$c1 * matrix(runif(n*control$s,0,1), ncol=control$s) * ( lb.s - pos.s ) +
      control$c2 * matrix(runif(n*control$s,0,1), ncol=control$s) * ( gb.s - pos.s )
    
    # new position
    pos.s <- pos.s + vel.s
    
    # cardinality
    pos.s <- make_sparse(pos=pos.s, sum_wgt=control$sum_wgt, cardinal_n=control$cardinal_n, NAV=v$options$NAV, prices=prices)
    ...
    # init local best
    lb.c[, lb.fit > fit.s] <- pos.c[, fit.s < lb.fit]
    lb.s[, lb.fit > fit.s] <- pos.s[, fit.s < lb.fit]
    # init fittness of all local best
    lb.fit[lb.fit > fit.s] <- fit.s[fit.s < lb.fit]
    ...
    # init local best
    lb.c[, lb.fit > fit.c2] <- pos.c2[, fit.c2 < lb.fit]
    lb.s[, lb.fit > fit.c2] <- pos.s[, fit.c2 < lb.fit]
    # init fittness of all local best
    lb.fit[lb.fit > fit.c2] <- fit.c2[fit.c2 < lb.fit]
...
csvpso_wrapper <- function(v, save_stats = FALSE){
  env <- new.env()
